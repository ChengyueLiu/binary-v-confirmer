...
for data_item in tqdm(data_items[:100]):
    ...
    # 打印原始的asm_code
    print(asm_code)
    # asm_code_words = re.split(r'[,\s]\s*', asm_code)
    # asm_words.extend(asm_code_words)
    # 使用 tokenizer.convert_ids_to_tokens 将 token IDs 转换回 tokens
    tokens = tokenizer.convert_ids_to_tokens(encoding['input_ids'][0])
    print(tokens, "\n")
# counter = Counter(asm_words)
# words = []
