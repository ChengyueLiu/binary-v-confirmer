...
class VulConfirmTeam:
    def __init__(self, function_confirm_model_pth_path=r"Resources/model_weights/model_1_weights.pth",
                 snippet_positioning_model_CGB_pth_path=r"Resources/model_weights/model_2_weights_GCB.pth",
                 snippet_positioning_model_BB_pth_path=r"Resources/model_weights/model_2_weights_bigbird.pth",
        ...
        # self.snippet_positioner = SnippetPositioner(model_save_path=snippet_positioning_model_CGB_pth_path,
        #                                             batch_size=batch_size)
        self.snippet_positioner = SnippetPositioner_BB(model_save_path=snippet_positioning_model_BB_pth_path,
                                                       batch_size=batch_size)
    ...
    def confirm_snippet(self, cause_function, possible_bin_function, is_vul=True):
        ...
        if cause_function.function_name == "PKCS12_unpack_p7data" or cause_function.function_name == "*PKCS12_unpack_p7data":
            print()
        # 定位代码片段
        ...
        for cause_function in vul.cause_functions:
            ...
            for i, possible_bin_function in enumerate(cause_function.possible_bin_functions, start=1):
                ...
                if not vul_asm_codes_window_texts:
                    logger.warning(f"patch_asm_codes_window_texts is empty, {possible_bin_function.function_name}")
