...
wt_patch_tab AS
(
    SELECT tt.vulnerability_id,
           string_agg(sp1.url, chr(13)) AS url,
           string_agg(tt.patch_hash::text, chr(13)) AS patch_hashes -- 新增行，聚合patch_hash
    FROM
    (
...
SELECT sl3."name", sl3.vendor, sl3.platform, slv2.version_number, vrug.repo_url,
       wct.cve_number, wct.cve_link, wct.publish_date, wct.description, --没有 title , severity 没有直接的值，需要写个判断
       svl.version_ranges, svl.exclude_versions, wpt.url, wpt.patch_hashes -- 修改此行以包括patch_hashes
FROM wt_cve_tab wct
