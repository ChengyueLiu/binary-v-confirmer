...
def prepare_train_data_for_model_1_from_million_funcs():
    ...
    save_to_json_file([data_item.custom_serialize() for data_item in all_data_items], train_data_save_path,
                      output_log=True)
    ...
    save_to_json_file([data_item.custom_serialize() for data_item in all_data_items], val_data_save_path,
                      output_log=True)
    logger.info(f"done, data_items_num: {len(all_data_items)}")
    ...
    save_to_json_file([data_item.custom_serialize() for data_item in all_data_items], test_data_save_path,
                      output_log=True)
    logger.info(f"done, data_items_num: {len(all_data_items)}")
...
def train_model_1():
    ...
    """
    下一轮计划：正负1:3，漏洞函数10倍，负例相似度限制在0.4-0.9之间，源代码行数提升到了7行。目的是提高对相似函数的识别能力。
    
    model_1_weights: 正负1:3，漏洞函数10倍，200万组件训练的结果
    model_1_weights_back_2: 正负1:1,140万个组件函数训练出来的结果
    model_1_weights_back: 使用75万个独立的函数训练的结果
    """
    back_model_save_path = r"Resources/model_weights/model_1_weights_back_2.pth"
